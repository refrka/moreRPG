class_name HiddenButton extends PanelContainer
...
var font_color: Color:
	set(value):
		if is_instance_valid(%HiddenButtonLabel):
			%HiddenButtonLabel.add_theme_color_override("font_color", value)
		font_color = value
...
func _ready() -> void:
	...
	_hover(false)
	%HiddenButtonLabel.text = button_text
	if hoverable:
		_connect_hover_signals()
	if clickable:
		_connect_click_signal()
